 ...
 *
 * `animated` opts into the motion defined in logo.css — meant for large
 * placements (hero); it self-disables under `prefers-reduced-motion`.
 */
export function AppLogo({ className, animated }: { className?: string; animated?: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={cn("size-5 shrink-0 text-primary", animated && "app-logo-animated", className)}
      aria-hidden
    >
      {/* Solid base carrying an open shape — the groundwork is done, the product is yours. */}
      <g className="app-logo-shape">
        <rect
          x="6.75"
          y="3.75"
          width="10.5"
